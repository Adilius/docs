...
=== "Environment (Docker Compose)"
    ```yaml
    environment:
      FTLCONF_dns_CNAMEdeepInspect: true
    ```
...
=== "Environment (Docker Compose)"
    ```yaml
    environment:
      FTLCONF_dns_analyzeOnlyAandAAAA: false
    ```
...
=== "CLI"
    ```shell
    sudo pihole-FTL --config dns.domain="lan"
    ```
=== "Environment (Docker Compose)"
    ```yaml
    environment:
...
=== "Environment (Docker Compose)"
    ```yaml
    environment:
      FTLCONF_dhcp_rapidCommit: false
    ```
...
=== "Environment (Docker Compose)"
    ```yaml
    environment:
      FTLCONF_resolver_networkNames: true
    ```
...
=== "Environment (Docker Compose)"
    ```yaml
    environment:
      FTLCONF_database_DBimport: true
    ```
...
=== "CLI"
    ```shell
    sudo pihole-FTL --config webserver.headers='["X-DNS-Prefetch-Control:off","Content-Security-Policy:default-src'self''unsafe-inline';","X-Frame-Options:DENY","X-XSS-Protection:0","X-Content-Type-Options:nosniff","Referrer-Policy:strict-origin-when-cross-origin"]'
    ```
=== "Environment (Docker Compose)"
    ```yaml
    environment:
...
=== "Environment (Docker Compose)"
    ```yaml
    environment:
      FTLCONF_webserver_serve_all: false
    ```
...
=== "Environment (Docker Compose)"
    ```yaml
    environment:
      FTLCONF_misc_extraLogging: false
    ```
...
=== "Environment (Docker Compose)"
    ```yaml
    environment:
      FTLCONF_debug_networking: false
    ```
...
=== "Environment (Docker Compose)"
    ```yaml
    environment:
      FTLCONF_debug_aliasclients: false
    ```
